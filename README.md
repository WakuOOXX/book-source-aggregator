...
- 一次并发扫遍所有有效书源，结果边出边上屏，不用干等。
- 直搜没结果会自动换关键词变体重试；「只看相关」能挡住那些无视搜索词、甩一堆热门书凑数的小站。
- 带 JS 的书源能搜（内置 V8 跑 Legado 的 `java.*` 脚本）；要登录的源自动跳过，不会卡住整轮。

**选书下载**

- 结果表选择和资源管理器一样：点选、Ctrl 加选、Shift 连选、空白处按住拖出绿框框选；也有全选 / 反选 / 清空，重开程序记得上次勾了哪些。
- 两种模式：**万里挑一**（勾的书依次换源试下，第一本成功就停）/ **全部下载**（每本各存一份）。
- 导出格式：**自动**（优先 EPUB，源不支持就降 TXT）/ EPUB / TXT。
- 正文一章章抓，目录分页拼全；重名的书自动带书源后缀，不会互相覆盖。

**书源管理**

- 多个书源文件自动合并去重；384 并发连通性体检生成有效表；深度校验（真发一次搜索 + 探分类页）给每个源打质量等级。
- 书源按 Legado 顶层分组名自动归组，搜索时可以只搜某个分组。
- 要登录的源：CDP 驱动 Chrome 逐站登录，Cookie 和请求头自动抓回、注入对应书源。
...
**搜索**：输入书名或作者回车。书源分组下拉能限定只搜某组（先用书源页校验出「可用」组），域下拉默认「自动」，书名/作者/分类/简介任一项命中就留。结果表五列：书名 / 作者 / 分类 / 最新章节 / 书源。

**书源**：管书源 JSON 文件。导入的文件会复制进程序数据目录，进清单就参与搜索。校验书源对每个文件做连通性体检，生成 `<文件名>.good.json` 有效表；深度校验再对活源试搜一次、探分类页，打出完整可用 > 可搜 > 搜索空转 > 试搜失败的等级，搜索页可勾「只搜试搜通过源」。

**下载**：搜索页勾好书，选格式和模式点下载。完了在「下载」页统一管理，按下载时间倒序，双击打开、列表删除、直达目录。

**登录头**：要登录的源在这抓 Cookie。程序驱动 Chrome 打开各站登录页，你正常登录，Cookie 和请求头自动抓回按站点存档。

**日志与设置**：主题切换、数据存储目录（换目录会自动把书源/下载/登录头搬过去并清理旧残留），还有两个别搞混的按钮：
